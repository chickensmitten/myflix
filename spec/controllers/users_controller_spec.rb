  ...
  describe "POST create" do
    ...
    context "sending emails" do
      ...
      it "sends out email to the user with valid inputs" do
        post :create, user: { email: "joe4@example.com", password: "password", full_name: "Joe Way"}
        expect(ActionMailer::Base.deliveries.last.to).to eq(['joe4@example.com'])
      end
      it "sends out email containing the user's name with valid inputs" do
        post :create, user: { email: "joe4@example.com", password: "password", full_name: "Joe Way"}
        expect(ActionMailer::Base.deliveries.last.body).to include("Joe Way")        
      end
      it "does not send out email with invalid inputs" do
        post :create, user: { email: "joe4@example.com"}
